...
Blockly.Procedures.allProcedures = function() {
  ...
  var proceduresReturnInteger = [];
  var proceduresReturnBoolean = [];
  var proceduresReturnString = [];
  ...
  for (var x = 0; x < blocks.length; x++) {
    ...
    if (func) {
      ...
      if (tuple) {
        if (tuple[2] == 'int') {
          proceduresReturnInteger.push(tuple);
        }
        else if (tuple[2] == 'bool') {
          proceduresReturnBoolean.push(tuple);
        }
        else if (tuple[2] == 'string') {
          proceduresReturnString.push(tuple);
        } else {
  ...
  proceduresReturnInteger.sort(Blockly.Procedures.procTupleComparator_);
  proceduresReturnBoolean.sort(Blockly.Procedures.procTupleComparator_);
  proceduresReturnString.sort(Blockly.Procedures.procTupleComparator_);
  return [proceduresNoReturn, proceduresReturnInteger, proceduresReturnBoolean, proceduresReturnString];
};
...
Blockly.Procedures.flyoutCategory = function(blocks, gaps, margin, workspace) {
  ...
  if (Blockly.Blocks['procedures_defreturn_integer']) {
    var block = Blockly.Block.obtain(workspace, 'procedures_defreturn_integer');
    block.initSvg();
    blocks.push(block);
    gaps.push(margin * 2);
  }
  if (Blockly.Blocks['procedures_defreturn_boolean']) {
    var block = Blockly.Block.obtain(workspace, 'procedures_defreturn_boolean');
    block.initSvg();
    blocks.push(block);
    gaps.push(margin * 2);
  }
  if (Blockly.Blocks['procedures_defreturn_string']) {
    var block = Blockly.Block.obtain(workspace, 'procedures_defreturn_string');
    block.initSvg();
    blocks.push(block);
    gaps.push(margin * 2);
  }
  ...
  populateProcedures(tuple[1], 'procedures_callreturn_integer');
  populateProcedures(tuple[2], 'procedures_callreturn_boolean');
  populateProcedures(tuple[3], 'procedures_callreturn_string');
};
