...
function getBlocksTop(block, list) {
  var children = block.getChildren();
  for(var i=0; i<children.length; i++){
    list.push(children[i]);
    if(children[i].getChildren().length > 0){
      getBlocksTop(children[i], list);
    }
  }
  return list;
}
...
Blockly.JavaScript.init = function(block) {
  ...
  var list_temp = [];
  var events_name = [];
  var blocks = getBlocksTop(block, list_temp);
  var header_code = '';
  var shared_code = '';
  var protocol = '';

  var setted_network = false;

  for (var x = 0; x < blocks.length; x++) {

    if(blocks[x].type == 'init_shared_integer') {
      var text_name = blocks[x].getFieldValue('VAR');
      var value_var = Blockly.JavaScript.valueToCode(blocks[x], 'VALUE', Blockly.JavaScript.ORDER_ATOMIC);
      shared_code += 'static short '+text_name+';\n';
    }
    if(blocks[x].type == 'init_shared_boolean') {
      var text_name = blocks[x].getFieldValue('VAR');
      var value_var = Blockly.JavaScript.valueToCode(blocks[x], 'VALUE', Blockly.JavaScript.ORDER_ATOMIC);
      shared_code += 'static boolean '+text_name+';\n';
    }
    if(blocks[x].type == 'init_shared_string') {
      var text_name = blocks[x].getFieldValue('VAR');
      var value_var = Blockly.JavaScript.valueToCode(blocks[x], 'VALUE', Blockly.JavaScript.ORDER_ATOMIC);
      shared_code += 'static String '+text_name+';\n';
    }

    if(blocks[x].type == 'select_protocol') {
      var dropdown_protocol = blocks[x].getFieldValue('PROTOCOL');
      protocol = 'static public Network network = new Network(Protocol.'+dropdown_protocol+');\n';
    }

    if(blocks[x].type == 'read_dios' || blocks[x].type == 'write_dios') {
      header_code += 'DigitalPin digit[] = new DigitalPin[4];\ndigit[0] = new DigitalPin(false, DigitalPin.DIO0);\ndigit[1] = new DigitalPin(false, DigitalPin.DIO1);\ndigit[2] = new DigitalPin(false, DigitalPin.DIO2);\ndigit[3] = new DigitalPin(false, DigitalPin.DIO3);\n';
    }

    if(blocks[x].type == 'trigger_event') {
      ...
      if (func) {
        ...
        for (var y = 0; y < blockVariables.length; y++) {
          ...
          if(!isInArray(varName, events_name)) {
            events_name.push(varName);
            header_code += 'Semaphore sem_'+varName+' = new Semaphore((short)0);\nEventThread_'+varName+' th_'+varName+' = new EventThread_'+varName+'(sem_'+varName+');\nth_'+varName+'.start();\n';
          }
        }
      }
    }

    if(blocks[x].type == 'wait_event') {
      var func = blocks[x].getVars;
  ...
  return [header_code, shared_code, protocol];
};
